...
def test_create_media_resource(api_client, api_rf, file, km_user_factory):
    ...
    data = {"file": file, "name": "Test Media Resource"}
    ...
    serializer = serializers.MediaResourceSerializer(
        km_user.media_resources.get(), context={"request": request}
    )
...
def test_get_media_resource_list(
    api_client, api_rf, km_user_factory, media_resource_factory
):
    """
    ...
    serializer = serializers.MediaResourceSerializer(
        km_user.media_resources.all(), context={"request": request}, many=True
    )
