...
def test_create(km_user_factory):
    ...
    data = {"email": "test@example.com", "is_admin": True}
...
def test_create_failed(km_user_factory):
    ...
    with mock.patch.object(
        km_user, "share", side_effect=ValidationError("foo")
    ) as mock_share:
        with pytest.raises(SerializerValidationError) as ex_info:
            serializer.save(km_user=km_user)

    assert mock_share.call_count == 1
    assert ex_info.value.detail == {api_settings.NON_FIELD_ERRORS_KEY: "foo"}
...
def test_serialize(
    ...
    accessor = km_user_accessor_factory(
        email=user.primary_email.email, km_user=km_user, user_with_access=user
    )
    ...
    user_serializer = UserInfoSerializer(
        accessor.user_with_access, context={"request": request}
    )
